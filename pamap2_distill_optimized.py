...
print("=" * 60)
print("MiniMax知识蒸馏 - 优化版")
print("=" * 60)

# Load data
print("\n[1] Loading PAMAP2 (ALL data)...")
...
- Walking: positive Y-trend, regular pattern
- Jogging: high magnitude, rapid steps
...
X_train, X_test, y_train, y_test = train_test_split(
    X_norm, y, test_size=0.2, random_state=42, stratify=y  # 80/20 split
)
print(f"Train: {len(X_train)}, Test: {len(X_test)}")

# Step 3: Generate soft labels (100 per class for efficiency)
print("\n[3] Generating soft labels (100 per class)...")
...
for class_id in range(NUM_CLASSES):
    ...
    print(f"Generating soft labels for {CN_ACTIVITY[class_id]}: {n} samples")
    
    for i, idx in enumerate(sampled):
        soft_label, _ = get_soft_label(X_train[idx], y_train[idx])
...
print(f"Generated soft labels for training data")

# MixUp Data Augmentation
def mixup_data(x, y, y_soft, alpha=0.4):
    """MixUp数据增强"""
    lam = np.random.beta(alpha, alpha)
    batch_size = x.size(0)
    index = torch.randperm(batch_size)
    mixed_x = lam * x + (1 - lam) * x[index]
    mixed_y_soft = lam * y_soft + (1 - lam) * y_soft[index]
    return mixed_x, y, mixed_y_soft
...
criterion = DistillationLoss(T=3.0, alpha=0.6)

best_acc, best_preds = 0, None
for epoch in range(150):  # 更多训练轮次
    model.train()
    
    # MixUp增强
    if epoch < 50:  # 前50轮使用MixUp
        perm = torch.randperm(len(X_train_t))
        X_mix, y_mix_hard, y_mix_soft = mixup_data(X_train_t[perm], y_train_t[perm], y_soft_train_t[perm], alpha=0.4)
        
        for i in range(0, len(X_mix), 64):
            outputs = model(X_mix[i:i+64])
            optimizer.zero_grad()
            loss = criterion(outputs, y_mix_hard[i:i+64], y_mix_soft[i:i+64])
            loss.backward()
            optimizer.step()
    else:
        perm = torch.randperm(len(X_train_t))
