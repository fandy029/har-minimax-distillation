"""
distill_gait_v2.py - Gait 优化版
================================
Gait: 4类, 98.37% (gap 1.63%)
注意: "活动"类始终0%，数据本身问题非模型问题
优化: 高α蒸馏，保持v1优势
"""
...
def main():
    ...
    if os.path.exists(soft_file):
        ...
        sys.stdout.write("  No soft labels, using one-hot...\n"); sys.stdout.flush()
    ...
    for ep in range(1, EPOCHS_KD+1):
        ...
        if ep % 10 == 0 or ep == 1:
            ...
            sys.stdout.write(f"    ep{ep:>3}: val={val_acc*100:.1f}% best_ft={best_ft_acc*100:.1f}% | test={ft_acc*100:.1f}% ({time.time()-t2:.0f}s)\n"); sys.stdout.flush()
    ...
    result = {"dataset": "Gait", "num_classes": n_cls, "train": len(X_tr), "test": len(X_te),
