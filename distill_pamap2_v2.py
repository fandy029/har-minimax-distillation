"""
distill_pamap2_v2.py - PAMAP2 优化版
======================================
PAMAP2: 5类, 93.1% (gap 6.9%)
优化: 高α蒸馏 + 各类别平衡采样
"""
...
def load_pamap2():
    ...
    d, l = [], []
    PAMAP_MAP = {9:0, 2:1, 3:2, 4:3, 5:4}  # downstairs, sit, stand, walk, jog
...
class DeepCNN(nn.Module):
    def __init__(self, in_ch=6, n_cls=5):
        super().__init__()
...
def main():
    ...
    if os.path.exists(soft_file):
        ...
        sys.stdout.write("  Generating soft labels for PAMAP2...\n"); sys.stdout.flush()
        ...
        for c in range(n_cls):
            ...
            sys.stdout.write(f"    Class {c}({cn[c]}): {n} samples\n"); sys.stdout.flush()
            for idx in sampled:
                y_soft[idx, y_tr[idx]] = 1.0  # fallback: one-hot
    ...
    for ep in range(1, EPOCHS_KD+1):
        ...
        if ep % 10 == 0 or ep == 1:
            ...
            sys.stdout.write(f"    ep{ep:>3}: val={val_acc*100:.1f}% best_ft={best_ft_acc*100:.1f}% | loss={total_loss/max(1,n_batches):.4f} ({(time.time()-t2):.0f}s)\n"); sys.stdout.flush()
            for c in range(n_cls): sys.stdout.write(f"      {cn[c]}: {ft_ca.get(cn[c],0)*100:.1f}%\n"); sys.stdout.flush()
    
    model.load_state_dict(best_ft_state)
    ft_acc, ft_ca = evaluate(model, Xte.to(DEVICE), yte, cn)
    
    with open("/home/fandy/workplace/thesis/all_dataset_results.json") as f:
        all_res = json.load(f)
    v1_acc = all_res['new'][0]['cnn_minimax'] if all_res['new'][0]['dataset'] == 'PAMAP2' else 93.1
    
    sys.stdout.write(f"\n{'='*55}\n"); sys.stdout.flush()
    sys.stdout.write(f"  Pure CNN: {pure_acc*100:.2f}%\n"); sys.stdout.flush()
    sys.stdout.write(f"  v1 KD:   {v1_acc:.2f}%\n"); sys.stdout.flush()
    sys.stdout.write(f"  v2 KD:   {ft_acc*100:.2f}%\n"); sys.stdout.flush()
    sys.stdout.write(f"  vs Pure:  {ft_acc*100-pure_acc*100:+.2f}%\n"); sys.stdout.flush()
    sys.stdout.write(f"  vs v1:   {ft_acc*100-v1_acc:+.2f}%\n\n"); sys.stdout.flush()
    
    for c in range(n_cls):
        name = cn[c]
        p = pure_ca.get(name, 0)
        k1 = v1_acc  # v1没有per-class，暂用总acc
        k2 = ft_ca.get(name, 0)
        diff = k2 - p
        marker = "✅" if diff > 0.02 else "❌" if diff < -0.02 else "  "
        sys.stdout.write(f"  {marker} {name:<15s}: Pure={p*100:5.1f}% v2={k2*100:5.1f}% ({diff:+.1f}%)\n"); sys.stdout.flush()
    
    result = {
        "dataset": "PAMAP2", "num_classes": n_cls, "train": len(X_tr), "test": len(X_te),
