"""
distill_uci_har_v2.py - UCI-HAR 优化版
======================================
UCI-HAR: 6类, 96.50% (gap 3.5%)
数据: 561维预处理特征，用MLP处理
"""
...
def main():
    ...
    if os.path.exists(soft_file):
        ...
        sys.stdout.write("  [WARN] Soft labels not found, using one-hot fallback\n"); sys.stdout.flush()
        y_soft = np.zeros((len(X_tr), 6), dtype=np.float32)
    ...
    for ep in range(1, EPOCHS_KD+1):
        ...
        if ep % 10 == 0 or ep == 1:
            ...
            sys.stdout.write(f"    ep{ep:>3}: val={val_acc*100:.1f}% best_ft={best_ft_acc*100:.1f}% | test={ft_acc*100:.1f}% loss={total_loss/max(1,n_batches):.4f} ({(time.time()-t2):.0f}s)\n"); sys.stdout.flush()
