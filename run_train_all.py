...
DISTILL_SCRIPT = 'scripts/run_distill.py'

ALL_DATASETS = ['pamap2', 'kuhar', 'uci_har', 'harth', 'uci_har_new',
                'motionsense', 'motionsense_dm', 'gait', 'wisdm']
...
def get_training_status():
    """获取所有数据集的训练状态（已完成的结果）"""
    history_dir = os.path.join(SCRIPT_DIR, 'results', 'history')
    status = {}
...
def main():
    ...
    for ds in datasets:
        ...
        for version in versions:
            ...
            history_file = os.path.join(SCRIPT_DIR, 'results', 'history', f'{ds}_{version}_history.json')
