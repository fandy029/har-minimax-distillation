...
def main():
    ...
    # 检查软标签文件
    import os
    soft_file = os.path.join(THESIS_DIR, "results", "soft_labels", f"{dataset}_soft.npy")
    checkpoint_file = os.path.join(THESIS_DIR, "results", "checkpoints", f"{dataset}_{version}_best.pt")
    ...
    # 调用训练脚本
    import subprocess
    script = os.path.join(THESIS_DIR, "scripts", "run_distill.py")
    cmd = [sys.executable, "-u", script, dataset, version]
    if resume:
        cmd.append('--resume')

    log_file = os.path.join(THESIS_DIR, "results", "logs", f"train_{dataset}_{version}.log")
    print(f"\n🚀 启动: {' '.join(cmd)}")
