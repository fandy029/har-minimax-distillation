...
def train_phase2(model_name, X_t, y_t, X_v, y_v, X_te, y_te, C, soft_t_all, soft_t_filtered, soft_t_correct):
    ...
    logger.info(f"{'='*60}\n阶段2: {model_name.upper()} | MOTIONSENSE | {strategy} α={alpha} T={T_val}\n硬{alpha*100:.0f}% | 软{(1-alpha)*100:.0f}% | 基线={baseline_test:.4f}\n{'='*60}")
...
if __name__ == '__main__':
    ...
    summary={'dataset':'MOTIONSENSE','phase':2,'timestamp':datetime.now().isoformat(),'best_params_per_model':BEST_PARAMS,'results':all_results}
