...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python gen_soft_labels_unified.py <dataset> [--ratio 0.35] [--limit 400] [--force]")
        print(f"可用数据集: {list(DATASET_CONFIG.keys())}")
        print("  --ratio: 每类采样率，默认 0.35")
        print("  --limit: 每类软标签上限，默认 400")
    ...
    if len(sys.argv) > 2 and not sys.argv[2].startswith('--'):
        ...
        print(f"  各class软标签(35%上限400): {dynamic_spc}")
