...
def train_phase2(model_name, X_t, y_t, X_v, y_v, X_te, y_te, C, soft_t_all, soft_t_filtered, soft_t_correct):
    ...
    logger.info(f"{'='*60}\n阶段2: {model_name.upper()} | HARTH | {strategy} α={alpha} T={T_val}\n硬{alpha*100:.0f}% | 软{(1-alpha)*100:.0f}% | 基线={baseline_test:.4f}\n{'='*60}")
...
if __name__ == '__main__':
    ...
    print(f"{'='*60}\n加载 HARTH 数据...")
        base = os.path.join(THESIS_DIR, 'datasets', 'HARTH')
    X_all, y_all = [], []
    for f in sorted(glob(os.path.join(base, '**', '*.csv'), recursive=True)):
        try:
            df = pd.read_csv(f)
            sensor = df[['back_x','back_y','back_z','thigh_x','thigh_y','thigh_z']].values.astype(np.float32)
            ...
            for s in range(0, len(df)-127, 64):
                ...
                if w.shape[0]==128 and not np.any(np.isnan(w)):
                    ...
                    if label in LABEL_MAP: X_all.append(w); y_all.append(LABEL_MAP[label])
        except: continue
    ...
    summary={'dataset':'HARTH','phase':2,'timestamp':datetime.now().isoformat(),'best_params_per_model':BEST_PARAMS,'results':all_results}
