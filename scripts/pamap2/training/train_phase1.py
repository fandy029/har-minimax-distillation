...
def full_evaluate(model, X, y, device, batch_size=256):
    model.eval()
    ds = TensorDataset(X, y); dl = DataLoader(ds, batch_size=batch_size)
...
def train_phase1(model_name, X_t, y_t, X_v, y_v, X_te, y_te, C):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"阶段1: {model_name.upper()} 预训练 | PAMAP2")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"阶段1 完成!  train={train_acc_f:.4f}  val={best_val_acc:.4f} @ E{best_epoch}  test={test_acc:.4f}  {elapsed:.1f}min")
    ...
    results = {'model': model_name, 'phase': 1, 'n_params': n_params,
        ...
        'config': {'loss': 'FocalLoss(γ=2.0)', 'optimizer': 'AdamW(lr=5e-4,wd=1e-4)',
                   'scheduler': 'CosineAnnealingWarmRestarts(T0=20,Tmult=2)',
                   'batch_size': BATCH_SIZE, 'patience': PATIENCE, 'max_epochs': MAX_EPOCHS,
                   'grad_clip': CLIP_GRAD, 'seed': SEED}, 'epoch_log': epoch_log}
...
if __name__ == '__main__':
    ...
    for c in range(N_CLS):
        print(f"  {CLASS_NAMES[c]:<12s}: train={int((y==c).sum())} val={int((y_vl==c).sum())} test={int((y_te==c).sum())}")
    ...
    summary = {'dataset': 'PAMAP2', 'phase': 1, 'timestamp': datetime.now().isoformat(),
    ...
    for m, r in all_results.items():
        print(f"  {m:<14s}: val={r['val_acc']:.4f} test={r['test_acc']:.4f} params={r['n_params']:,} time={r['time_min']:.1f}min")
    print(f"{'='*60}")
