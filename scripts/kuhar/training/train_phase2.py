...
BEST_PARAMS = {
    'purecnn':     {'alpha': 0.5, 'T': 2.5, 'strategy': 'all', 'label': 'all_a0.5_T2.5'},
...
DATASET = 'kuhar'
CLASS_NAMES = ['Stand', 'Sit', 'Talk-sit', 'Talk-stand', 'Stand-sit', 'Lay', 'Lay-stand', 'Pick', 'Jump', 'Push-up', 'Sit-up', 'Walk', 'Walk-backwards', 'Walk-circle', 'Run', 'Stair-up', 'Stair-down', 'Table-tennis']
N_CLS = len(CLASS_NAMES)



BATCH_SIZE, PHASE2_LR, PATIENCE, MAX_EPOCHS, CLIP_GRAD, SEED = 64, 1e-4, 15, 300, 5.0, 42
DEVICE_STR = 'cpu'
OUT_DIR = os.path.join(GAIT_DIR, 'output')
RESULTS_DIR = os.path.join(OUT_DIR, 'results'); SOFT_DIR = os.path.join(OUT_DIR, 'soft_labels')
...
def train_epoch_distill(model, loader, optimizer, device, soft_labels, alpha, T, clip_grad=5.0):
    ...
    for xb, soft_b, yb in loader:
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), clip_grad); optimizer.step()
...
def train_phase2(model_name, X_t, y_t, X_v, y_v, X_te, y_te, C, soft_t_all, soft_t_filtered, soft_t_correct):
    ...
    logger.info(f"{'='*60}\n阶段2: {model_name.upper()} | KUHAR | {strategy} α={alpha} T={T_val}\n硬{alpha*100:.0f}% | 软{(1-alpha)*100:.0f}% | 基线={baseline_test:.4f}\n{'='*60}")
    ...
    best_val_acc, best_state, best_epoch, counter = 0.0, None, 0, 0
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if val_acc > best_val_acc:
            best_val_acc, best_epoch = val_acc, epoch+1; best_state = copy.deepcopy(model.state_dict()); counter = 0
        else:
            counter += 1
    ...
    logger.info(f"\n{'='*60}\n完成! train={train_acc_f:.4f} val={best_val_acc:.4f} @E{best_epoch}\ntest={test_acc:.4f} (基线{baseline_test:.4f}, {'+' if gain>0 else ''}{gain*100:.2f}pp) {elapsed:.1f}min")
    ...
    logger.info(f"{'='*60}")
    results = {'model': model_name, 'phase': 2, 'strategy': strategy, 'alpha': alpha, 'T': T_val,
        ...
        'config': {'loss': 'α·Focal+(1-α)·T²·KL', 'alpha': alpha, 'T': T_val,
            'optimizer': 'AdamW(lr=1e-4,wd=1e-4)', 'scheduler': 'CosWR(T0=20)',
...
if __name__ == '__main__':
    ...
    print(f"{'='*60}\n加载 KUHAR 数据...")
        base = os.path.join(THESIS_DIR, 'datasets', 'KuHar', '1.Raw_time_domian_data')
    X_all, y_all = [], []
    ...
    C=X_t.shape[1]
    soft_all=torch.FloatTensor(np.load(os.path.join(SOFT_DIR, f'{DATASET}_soft_all.npy')))
    soft_filtered=torch.FloatTensor(np.load(os.path.join(SOFT_DIR, f'{DATASET}_soft_filtered.npy')))
    soft_correct=torch.FloatTensor(np.load(os.path.join(SOFT_DIR, f'{DATASET}_soft_correct_only.npy')))
    all_results={}
    for model_name in MODELS:
        phase1_pt=os.path.join(RESULTS_DIR,f'{DATASET}_{model_name}_phase1.pt')
        if not os.path.exists(phase1_pt): print(f"⚠️  {model_name}: 无阶段1权重,跳过"); continue
    ...
    summary={'dataset':'KUHAR','phase':2,'timestamp':datetime.now().isoformat(),'best_params_per_model':BEST_PARAMS,'results':all_results}
    with open(os.path.join(RESULTS_DIR,'phase2_summary.json'),'w') as f: json.dump(summary,f,indent=2,default=str)
    print(f"\n{'='*60}\n阶段2 完成!")
    for m,r in all_results.items(): print(f"  {m:<14s}: test={r['test_acc']:.4f} gain={r['gain_pct']} time={r['time_min']:.1f}min")
