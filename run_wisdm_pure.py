"""WISDM pure CNN - saturated dataset"""
...
def load_wisdm():
    base = '/home/fandy/workplace/thesis/datasets/WISDM/WISDM_ar_v1.1'
    d, l = [], []
    content = open(f"{base}/WISDM_ar_v1.1_transformed.arff").read()
    for line in content.split('\n'):
        if line.strip().startswith('@') or not line.strip(): continue
        parts = line.strip().split(',')
        if len(parts) < 46: continue
        try:
            # 30 features: X0-X9 (0-9), Y0-Y9 (10-19), Z0-Z9 (20-29)
            x_vals = [float(x) for x in parts[2:12]]   # X0-X9
            y_vals = [float(x) for x in parts[12:22]]  # Y0-Y9
            z_vals = [float(x) for x in parts[22:32]]  # Z0-Z9
            # Stack as (3, 10) - 3 channels, 10 time steps
            window = [x_vals, y_vals, z_vals]
            label = parts[45].strip().strip('"')
            lbl_map = {'Walking':0,'Jogging':1,'Upstairs':2,'Downstairs':3,'Sitting':4,'Standing':5}
            if label in lbl_map:
                d.append(window); l.append(lbl_map[label])
        except: pass
    d = d[:10000]; l = l[:10000]
