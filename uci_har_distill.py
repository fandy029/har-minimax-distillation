...
WINDOW_SIZE = 128
NUM_CLASSES = 6
# 类别映射: UCI-HAR labels 1-6 -> 0-5
CN_ACTIVITY = {0:'步行', 1:'上楼', 2:'下楼', 3:'坐着', 4:'站立', 5:'躺下'}

# 类别权重（根据样本分布）
CLASS_WEIGHT = torch.FloatTensor([1.0, 1.2, 1.2, 1.0, 1.0, 1.0])

print("=" * 60)
print("UCI-HAR + MiniMax知识蒸馏")
...
def load_uciHAR():
    """加载UCI-HAR数据集"""
    train_dir = os.path.join(DATA_DIR, "train", "Inertial Signals")
    test_dir = os.path.join(DATA_DIR, "test", "Inertial Signals")
    
    # 加载训练数据
    def load_signals(dir_path, prefix, suffix):
        signals = []
        for axis in ['x', 'y', 'z']:
            fname = f"{prefix}_{axis}{suffix}.txt"
            with open(os.path.join(dir_path, fname)) as f:
                signals.append(np.loadtxt(f))
        return np.array(signals)  # (3, n_samples, 128)
    
    # body_acc, body_gyro, total_acc (每个x,y,z)
    prefixes = ['body_acc', 'body_gyro', 'total_acc']
    suffix = '_train'
    train_signals = []
    for p in prefixes:
        train_signals.append(load_signals(train_dir, p, suffix))
    train_signals = np.concatenate(train_signals, axis=0)  # (9, n_train, 128)
    
    suffix = '_test'
    test_signals = []
    for p in prefixes:
        test_signals.append(load_signals(test_dir, p, suffix))
    test_signals = np.concatenate(test_signals, axis=0)  # (9, n_test, 128)
    
    # 加载标签
    with open(os.path.join(DATA_DIR, "train", "y_train.txt")) as f:
        y_train = np.loadtxt(f, dtype=int) - 1  # 转成0-5
    with open(os.path.join(DATA_DIR, "test", "y_test.txt")) as f:
        y_test = np.loadtxt(f, dtype=int) - 1
    
    # 转换格式: (n_samples, 128, 9)
    X_train = np.transpose(train_signals, (1, 2, 0)).astype(np.float32)
    X_test = np.transpose(test_signals, (1, 2, 0)).astype(np.float32)
...
print(f"Test distribution: {np.bincount(y_test, minlength=NUM_CLASSES)}")
...
def extract_features(data):
    ...
    # Y轴（垂直方向）用total_acc的y
    y_accel = total_acc[:, 1]
    acc_mag = np.sqrt(np.sum(total_acc**2, axis=1))
    gyro_mag = np.sqrt(np.sum(body_gyro**2, axis=1))
    ...
    dom_freq = dom_freq_idx * (1.0 / WINDOW_SIZE) * 100  # 估计频率
    ...
    return {
        ...
        'body_acc_mag_mean': body_acc_mag.mean(),
...
def get_soft_label(data, true_label):
    ...
    prompt = f"""Classify this IMU window. Classes: 0=walking, 1=upstairs, 2=downstairs, 3=sitting, 4=standing, 5=laying

Features:
- Y-trend: {f['y_trend']:.4f} (upward=positive, downward=negative)
- Step peaks: {f['step_peaks']}
- Total acc magnitude: {f['acc_mag_mean']:.2f} ± {f['acc_mag_std']:.2f}, max={f['acc_mag_max']:.2f}
- Body acc magnitude: {f['body_acc_mag_mean']:.2f}
- Y-range: {f['y_range']:.2f}
- Gyro magnitude: {f['gyro_mag_mean']:.3f}
...
- Laying: horizontal orientation, very low dynamic
...
np.save('/home/fandy/workplace/thesis/soft_labels_uci_har.npy', y_soft_train)
print(f"\nSaved soft labels to soft_labels_uci_har.npy")
