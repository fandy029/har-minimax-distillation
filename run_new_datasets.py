"""
只跑2个新数据集: HARTH, Gait_Classification
跑完一个就保存一次结果，避免丢失
"""
...
RESULT_FILE = '/home/fandy/workplace/thesis/new_dataset_results.json'

DATASETS = {
    'harth': {
        'Name': 'HARTH', 'path': '/home/fandy/workplace/thesis/datasets/HARTH/harth',
        'num_classes': 6, 'channels': 6,
        'cn': ['左立','走路','上楼','下楼','右立','站立'],
    },
    'gait': {
        'Name': 'Gait_Classification', 'path': '/home/fandy/workplace/thesis/datasets/Gait_Classification',
...
# 已知结果(不重跑)
KNOWN = {
    'pamap2':    {'Name':'PAMAP2',     'pure_cnn':92.7,'cnn_minimax':93.1,'num_classes':5,'train':2137,'test':535},
    'uci_har':   {'Name':'UCI-HAR',    'pure_cnn':96.2,'cnn_minimax':96.5,'num_classes':6,'train':7352,'test':2947},
    'motionsense':{'Name':'MotionSense','pure_cnn':99.2,'cnn_minimax':99.4,'num_classes':6,'train':17492,'test':4373},
    'wisdm':     {'Name':'WISDM',      'pure_cnn':99.6,'cnn_minimax':99.6,'num_classes':6,'train':13365,'test':3342},
}
...
LOADERS={'harth':load_harth,'gait':load_gait}
...
def save_results(result):
    """追加保存结果到文件"""
    existing={'new':[],'known':KNOWN}
    if os.path.exists(RESULT_FILE):
        try:
            with open(RESULT_FILE) as f: existing=json.load(f)
        except: pass
    # 更新new列表
    key=result['key']
    existing['new']=[r for r in existing.get('new',[]) if r.get('key')!=key]
    existing['new'].append(result)
    with open(RESULT_FILE,'w') as f: json.dump(existing,f,indent=2,ensure_ascii=False)
...
def run_ds(ds_key):
    ...
    if result is None: print("  加载失败"); return
    ...
    result_data={
        'key': ds_key,
        'dataset': cfg['Name'],
        'num_classes':n_cls,'train':len(X_tr),'test':len(X_te),
    ...
    print(f"\n  对比: Pure CNN {ap*100:.2f}% → CNN+MiniMax {ak*100:.2f}% ({(ak-ap)/ap*100:+.2f}%)")

def main():
    for ds in ['harth','gait']:
        run_ds(ds)
    ...
    if os.path.exists(RESULT_FILE):
        ...
        data={'new':[],'known':KNOWN}
    print(f"\n  {'数据集':<18} {'类':>3} {'训练':>6} {'PureCNN':>9} {'+MiniMax':>9} {'提升':>8}")
    print(f"  {'-'*55}")
    done={r['key']:r for r in data.get('new',[])}
    for k,v in sorted(KNOWN.items(),key=lambda x:-x[1]['pure_cnn']):
        print(f"  {v['Name']:<18} {v['num_classes']:>3} {v['train']:>6} {v['pure_cnn']:>8.1f}% {v['cnn_minimax']:>8.1f}% {'+'if v['cnn_minimax']>v['pure_cnn'] else ''}{v['cnn_minimax']-v['pure_cnn']:.1f}%")
    for k,v in sorted(done.items(),key=lambda x:-x[1].get('pure_cnn',0)):
        imp=v.get('improvement',0)
        print(f"  {v['dataset']:<18} {v['num_classes']:>3} {v['train']:>6} {v['pure_cnn']:>8.1f}% {v['cnn_minimax']:>8.1f}% {'+'if imp>0 else''}{imp:.1f}%")
    print(f"\n结果文件: {RESULT_FILE}")
