...
if __name__ == "__main__":
    ...
    soft_file = "/home/fandy/workplace/thesis/results/soft_labels/motionsense_soft.npy"
    ...
    pure_acc, _ = eval_model(model, Xte.to(DEVICE), y_te)
    ...
    for ep in range(1, 16):
        ...
        if ep % 5 == 0 or ep == 1:
            ft_acc, _ = eval_model(model, Xte.to(DEVICE), y_te)
            print(f"  ep{ep}: val={va*100:.1f}% test={ft_acc*100:.1f}%")
    
    model.load_state_dict(best_ft_state)
    ft_acc, ft_ca = eval_model(model, Xte.to(DEVICE), y_te)
    ...
    result = {"dataset": "MotionSense", "num_classes": 6, "train": len(X_tr), "test": len(X_te),
              ...
              "kd_class_acc": ft_ca}
    with open("/home/fandy/workplace/thesis/results/motion_sense_v1.json", "w") as f:
